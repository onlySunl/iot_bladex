...
- **SPI 接口默认实现**: `ChannelSmsStrategy`（短信）和 `ChannelVmsStrategy`（语音）是 SPI 接口，项目提供 `DefaultChannelSmsStrategy` 和 `DefaultChannelVmsStrategy` 作为空实现（仅打印日志）。用户可通过实现对应接口并提供自己的 `@Component` Bean 来覆盖默认实现（使用 `@ConditionalOnMissingBean` 确保优先级）。

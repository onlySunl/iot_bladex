 ...
 *
 * @author mqttsnet
 * @date 2023/9/30 5:16 下午
 */
public interface CachePlusOps extends CacheOps {

    /**
     * Redis Pub/Sub 发布消息到指定频道。
     * <p>用于多副本部署下的跨节点通知(如缓存失效广播)。
     * Caffeine 实现为空操作 + warn 日志(单 JVM 无意义)。
     *
     * @param channel 频道名,订阅方通过 RedisMessageListenerContainer 监听
     * @param message 消息内容(底层走 RedisTemplate.convertAndSend,String 透传,对象走默认序列化)
     */
    void publish(@NonNull String channel, @NonNull Object message);
    ...
    /**
     * 一次性把多个 field-value 写入哈希表 —— Redis 原生 HMSET / HMSet。
     * <p>用于批量初始化：单条 hSet 调 N 次会引发 N 次 round-trip，本方法一条命令搞定，
     *
     * @param hashKey     哈希主键（仅取 {@code key.getKey()}，本方法忽略 field）
     * @param fieldValues field → value 映射，至少一对
     */
    void hMSet(@NonNull CacheKey hashKey, @NonNull Map<? extends Object, ? extends Object> fieldValues);

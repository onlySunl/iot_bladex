...
public class CommonResult<T> implements Serializable {
    ...
    public static <T> CommonResult<T> error(IResultCode errorCode, Object... params) {
        Assert.notEquals(GlobalErrorCodeConstants.SUCCESS.getCode(), errorCode.getCode(), "code 必须是错误的！");
        CommonResult<T> result = new CommonResult<>();
        result.code = errorCode.getCode();
        result.msg = ServiceExceptionUtil.doFormat(errorCode.getCode(), errorCode.getMessage(), params);
        return result;
    }
    ...
    public static <T> CommonResult<T> error(ServiceException serviceException) {
        return error(serviceException.getResultCode(), serviceException.getMessage());
    }

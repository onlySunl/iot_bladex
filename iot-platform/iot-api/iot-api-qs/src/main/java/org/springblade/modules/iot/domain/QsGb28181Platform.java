 ...
 */
@Data
@TableName("qs_gb28181_platform")
@EqualsAndHashCode(callSuper = true)
@Table(value = "qs_gb28181_platform", comment = "国标GB28181平台配置表")
public class QsGb28181Platform extends CustomBaseEntity {
    ...
    @AutoColumn(comment = "是否启用：0-禁用，1-启用", length = 1, defaultValueType = DefaultValueEnum.NULL)
    ...
    @AutoColumn(comment = "注册有效期（秒）", length = 50, defaultValueType = DefaultValueEnum.EMPTY_STRING)
    private String expires;

    /** 心跳超时时间（秒） */
    @TableField(value = "keep_timeout")
    @AutoColumn(comment = "心跳超时时间（秒）", length = 50, defaultValueType = DefaultValueEnum.EMPTY_STRING)
    private String keepTimeout;

    /** 传输协议：UDP/TCP */
    @TableField(value = "transport")
    @AutoColumn(comment = "传输协议：UDP/TCP", length = 50, defaultValueType = DefaultValueEnum.EMPTY_STRING)
    ...
    @AutoColumn(comment = "状态：0-离线，1-在线", length = 1, defaultValueType = DefaultValueEnum.NULL)

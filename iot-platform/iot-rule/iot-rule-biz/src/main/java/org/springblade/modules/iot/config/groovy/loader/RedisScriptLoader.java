package org.springblade.modules.iot.config.groovy.loader;
...
public class RedisScriptLoader implements ScriptLoader {
    ...
    public ScriptEntry load(ScriptQuery query) throws Exception {
        if (StrUtil.isBlank(query.getUniqueKey())) {
            throw new ServiceException("脚本唯一键不能为空");
        }
        ...
        String script = ruleCacheDataHelper.getScriptContent(cacheKey,
                ...
                .map(Object::toString)
                .orElseThrow(() -> new ServiceException("脚本不存在"));
        // 获取脚本指纹
     ...
     * @return 包含编译后类的脚本实体
     * @throws ServiceException 当内容为空或编译失败时抛出
     */

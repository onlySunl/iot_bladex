...
import org.springblade.modules.iot.bridge.config.SubscriptionSourcePubSubConfig;
...
import org.springblade.modules.iot.bridge.event.source.SubscriptionSourceChangedEventSource;
...
public class SubscriptionSourceServiceImpl
        extends BladeServiceImpl<SubscriptionSourceManager, SubscriptionSource>
    ...
    public List<SubscriptionSourceResultVO> getSubscriptionSourceResultVOList(SubscriptionSourcePageQuery query) {
        List<SubscriptionSourceResultVO> voList = BeanPlusUtil.copyToList(
                baseMapper.getSubscriptionSourceList(query), SubscriptionSourceResultVO.class);
    ...
    public SubscriptionSourceResultVO getSubscriptionSourceDetail(Long id) {
        SubscriptionSource entity = baseMapper.getById(id);
    ...
    public SubscriptionSourceSaveVO saveSubscriptionSource(SubscriptionSourceSaveVO saveVO) {
        ...
        if (!ALLOWED_DIRECTIONS.contains(ds.getDirection())) {
            throw new ServiceException("订阅源关联的数据源方向必须为入站(20)或双向(30)");
        }
        ...
        ArgumentAssert.isTrue(baseMapper.save(entity), "订阅源保存失败");
    ...
    public SubscriptionSourceUpdateVO updateSubscriptionSource(SubscriptionSourceUpdateVO updateVO) {
        ...
        ArgumentAssert.notNull(baseMapper.getById(updateVO.getId()), "订阅源不存在或已被删除");
        ...
        ArgumentAssert.isTrue(baseMapper.updateById(entity), "订阅源更新失败");
    ...
    public boolean changeStatus(Long id, Boolean enable) {
        SubscriptionSource src = baseMapper.getById(id);
        ...
        boolean ok = baseMapper.updateById(src);
    ...
    public boolean deleteSubscriptionSource(Long id) {
        ...
        if (Boolean.TRUE.equals(src.getEnable())) {
            throw new ServiceException("启用中的订阅源不允许删除,请先禁用后再删");
        }
    ...
    public SubscriptionSource getByCode(String sourceCode) {
        return baseMapper.getByCode(sourceCode);
    }
    ...
    public void onDataSourceDeleteCheck(DataSourceDeleteCheckEvent event) {
        ...
        long subCount = baseMapper.count(Wrappers.<SubscriptionSource>lambdaQuery()
                .eq(SubscriptionSource::getDataSourceId, dsId));
        if (subCount > 0) {
            throw new ServiceException("此数据源被 " + subCount + " 个订阅源引用,请先删除关联订阅源");
        }
    }

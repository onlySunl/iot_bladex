...
import org.springblade.modules.iot.service.execution.service.RuleExecutionService;
import org.springblade.modules.iot.service.execution.trigger.RuleEffectiveWindowChecker;
...
import org.springblade.modules.iot.vo.result.linkage.RuleConditionActionDetailsResultVO;
import org.springblade.modules.iot.vo.result.linkage.RuleConditionDetailsResultVO;
import org.springblade.modules.iot.vo.result.linkage.RuleDetailsResultVO;
import org.springblade.modules.iot.vo.save.linkage.RuleSaveVO;
import org.springblade.modules.iot.vo.update.linkage.RuleUpdateVO;
...
public class RuleServiceImpl extends BladeServiceImpl<RuleManager, Rule> implements RuleService {
    ...
    private boolean saveRule(Rule rule) {
        try {
            return baseMapper.save(rule);
        } catch (Exception e) {
            log.error("Failed to save rule: {}", e.getMessage(), e);
            throw new ServiceException("Failed to save rule");
        }
    }
    ...
    public Rule updateRule(RuleUpdateVO updateVO) {
        ...
        Rule existingRule = Optional.ofNullable(baseMapper.getById(updateVO.getId()))
                .orElseThrow(() -> new ServiceException("The rule does not exist"));
    ...
    public Boolean deleteRule(Long id) {
        ...
        Rule rule = baseMapper.getById(id);
        if (null == rule) {
            throw new ServiceException("The Rule does not exist");
        }
    ...
    public Boolean updateRuleStatus(Long id, Integer status) {
        ...
        boolean updated = baseMapper.updateById(rule);
        if (updated) {
    ...
    public RuleDetailsResultVO getRuleDetails(Long id) {
        RuleDetailsResultVO ruleDetails = Optional.ofNullable(baseMapper.getById(id))
    ...
    public RuleDetailsResultVO getRuleDetailsByIdentification(String ruleIdentification) {
        RuleDetailsResultVO ruleDetails = Optional.ofNullable(baseMapper.findOneByRuleIdentification(ruleIdentification))
                .map(rule -> BeanPlusUtil.toBeanIgnoreError(rule, RuleDetailsResultVO.class))
                .orElseThrow(() -> new ServiceException("Rule not found"));
    ...
    private RuleDetailsResultVO doTriggerRulePolicy(String tenantId, String ruleIdentification) {
        try {
            ...
            if (Objects.isNull(ruleDetails)) {
                throw new ServiceException("ruleIdentification is not exist");
            }
        ...
        CacheResult<RuleDetailsResultVO> result = cachePlusOps.get(
                ...
                k -> {
                    try {
                        ...
                        // 查无此规则时底层抛 ServiceException:转 null 让空值缓存生效,而非每条消息都回源报错

...
import org.springblade.modules.iot.common.databridge.ConnectorConfig;
import org.springblade.modules.iot.common.databridge.ConnectorPayload;
import org.springblade.modules.iot.common.databridge.ConnectorType;
import org.springblade.modules.iot.common.databridge.SendResult;
import org.springblade.modules.iot.common.databridge.ConnectorRegistry;
import org.springblade.modules.iot.common.databridge.Sink;
import org.springblade.core.log.exception.ServiceException;
import org.springblade.core.tool.utils.JsonUtil;
import org.springblade.core.tool.utils.SnowflakeIdUtil;
import org.springblade.modules.iot.bridge.enrich.DataSourceInfoAttacher;
...
public class DataBridgeServiceImpl
        extends BladeServiceImpl<DataBridgeManager, DataBridge>
    ...
    public List<DataBridgeResultVO> getDataBridgeResultVOList(DataBridgePageQuery query) {
        List<DataBridgeResultVO> voList = BeanPlusUtil.copyToList(
                baseMapper.getDataBridgeList(query), DataBridgeResultVO.class);
    ...
    public DataBridgeResultVO getDataBridgeDetail(Long id) {
        DataBridge entity = baseMapper.getById(id);
    ...
    public DataBridgeSaveVO saveDataBridge(DataBridgeSaveVO saveVO) {
        ...
        ArgumentAssert.isTrue(baseMapper.save(entity), "桥接规则保存失败");
    ...
    public DataBridgeUpdateVO updateDataBridge(DataBridgeUpdateVO updateVO) {
        ...
        ArgumentAssert.notNull(baseMapper.getById(updateVO.getId()), "桥接规则不存在或已被删除");
        ...
        ArgumentAssert.isTrue(baseMapper.updateById(entity), "桥接规则更新失败");
        bridgeEventPublisher.publishRuleChangedEvent(entity);
        return updateVO;
    }
    ...
    public Map<String, Object> testSink(Long id, Map<String, Object> sampleEnvelope) {
        DataBridge rule = baseMapper.getById(id);
        ...
        if (!Boolean.TRUE.equals(ds.getEnable())) {
            throw new ServiceException("关联数据源未启用,请先启用数据源后再测试");
        }
        if (connectorRegistry == null) {
            throw new ServiceException("数据桥接 starter 未启用,无法测试发送");
        }
    ...
    public boolean changeStatus(Long id, Boolean enable) {
        ...
        if (Boolean.TRUE.equals(enable)) {
            ...
            ArgumentAssert.notNull(ds, "关联数据源不存在");
            if (!Boolean.TRUE.equals(ds.getEnable())) {
                throw new ServiceException("关联数据源未启用,请先启用数据源");
            }
        }
        rule.setEnable(enable);
        boolean ok = baseMapper.updateById(rule);
    ...
    public Long copyRule(Long id) {
        DataBridge src = baseMapper.getById(id);
        ...
        ArgumentAssert.isTrue(baseMapper.save(copy), "复制规则失败");
    ...
    public boolean deleteDataBridge(Long id) {
        ...
        if (Boolean.TRUE.equals(rule.getEnable())) {
            throw new ServiceException("启用中的规则不允许删除,请先禁用后再删");
        }
        boolean ok = baseMapper.removeById(id);
        if (ok) {
    ...
    public List<DataBridgeResultVO> getEnabledRules(String appId, String direction) {
        return BeanPlusUtil.copyToList(baseMapper.getEnabledRules(appId, direction),
    ...
    public void onDataSourceDeleteCheck(DataSourceDeleteCheckEvent event) {
        ...
        long ruleCount = baseMapper.count(Wrappers.<DataBridge>lambdaQuery()
                .eq(DataBridge::getDataSourceId, dsId));
        if (ruleCount > 0) {
            throw new ServiceException("此数据源被 " + ruleCount + " 条桥接规则引用,请先删除关联规则后重试");
        }
        long dlqCount = baseMapper.count(Wrappers.<DataBridge>lambdaQuery()
                .eq(DataBridge::getDeadLetterDataSourceId, dsId));
        if (dlqCount > 0) {
            throw new ServiceException("此数据源被 " + dlqCount + " 条桥接规则配为死信目标,请先调整规则的死信配置");
        }
    }
}

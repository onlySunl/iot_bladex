...
public class BridgeExecutionTraceServiceImpl
        extends BladeServiceImpl<BridgeExecutionTraceManager, BridgeExecutionTrace>
    ...
    public List<BridgeExecutionTraceResultVO> getTraceResultVOList(BridgeExecutionTracePageQuery query) {
        return BeanPlusUtil.copyToList(baseMapper.getTraceList(query), BridgeExecutionTraceResultVO.class);
    }

    @Override
    public BridgeExecutionTraceStatsResultVO getTraceStats(BridgeExecutionTracePageQuery query) {
        BridgeExecutionTraceStatsResultVO result = baseMapper.getTraceStats(query);
        result.setTimeline(buildTimeline(query));
        result.setTopRules(baseMapper.getTraceTopRules(query, TOP_RULE_LIMIT));
        return result;
    }
    ...
    public BridgeExecutionTraceResultVO getTraceDetail(String traceId, Long ruleId) {
        BridgeExecutionTrace trace = ruleId != null
                ? baseMapper.getByTraceIdAndRuleId(traceId, ruleId)
                : baseMapper.getByTraceId(traceId);
    ...
    public String replay(String traceId) {
        BridgeExecutionTrace trace = baseMapper.getByTraceId(traceId);
        ArgumentAssert.notNull(trace, "trace 不存在");
        if (!STATUS_DEAD_LETTER.equals(trace.getStatus())) {
            throw new ServiceException("仅死信状态(status=03)的 trace 可重放,当前状态:" + trace.getStatus());
        }
        if (trace.getBridgeRuleId() == null) {
            throw new ServiceException("trace 未关联桥接规则,无法重放(可能是订阅源拉取失败的入站消息)");
        }
    ...
    public boolean removeBefore(LocalDateTime cutoff) {
        return baseMapper.remove(Wrappers.<BridgeExecutionTrace>lambdaQuery()
    ...
    private List<BridgeExecutionTraceStatsResultVO.TimelinePoint> buildTimeline(BridgeExecutionTracePageQuery query) {
        Map<String, Object> bounds = baseMapper.getTraceTimeBounds(query);
        ...
        return baseMapper.getTraceTimeline(query, granularity.name());
    }

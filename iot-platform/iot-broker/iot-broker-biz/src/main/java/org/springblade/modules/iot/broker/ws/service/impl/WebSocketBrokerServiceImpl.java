...
public class WebSocketBrokerServiceImpl implements WebSocketBrokerService {
     ...
     * @throws ServiceException 设备不在线 / RocketMQ 不可用
     */
    @Override
    public String publishMessage(PublishWebSocketMessageRequestVO publishMessageRequestVO) throws ServiceException {
        ...
        if (!wsDeviceSessionRegistry.isOnline(tenantId, clientId)) {
            ...
            throw new ServiceException("设备不在线: " + clientId);
        }
    ...
    private void broadcast(PublishWebSocketMessageRequestVO vo, String clientId, String encodedJson) {
        ...
        if (rocketmqTemplate == null) {
            throw new ServiceException("RocketmqTemplate not available; cannot broadcast downlink clientId=" + clientId);
        }
    ...
    public String publishLocal(String clientId, String encodedJson) {
        ...
        if (subject == null) {
            throw new ServiceException("Local ws session not found: " + clientId);
        }
        try {
            ...
            throw new ServiceException("Push to local session failed: " + e.getMessage());
        }
    }

package org.springblade.modules.iot.broker.mqtt.service.impl;
...
import org.springblade.modules.iot.broker.mqtt.exception.SessionNotFoundException;
...
public class MqttBrokerServiceImpl implements MqttBrokerService {
    ...
    public String publishMessage(PublishMessageRequestVO publishMessageRequestVO) throws ServiceException {
        ...
        try {
            ...
            if (response.getStatusCode().is2xxSuccessful()) {
                ...
                throw new ServiceException("Failed to publish message with topic: " + publishMessageRequestVO.getTopic());
            }
        } catch (HttpClientErrorException e) {
            log.error("HTTP error occurred while publishing message: {}", e.getMessage());
            throw new ServiceException("Error during message publishing: " + e.getResponseBodyAsString());
        } catch (Exception e) {
            log.error("Unexpected error occurred while publishing message: {}", e.getMessage());
            throw new ServiceException("Unexpected error during message publishing: " + e.getMessage());
        }
    }
    ...
    public MqttSessionDetailsResultVO getSessionInfo(String tenantId, String userId, String clientId) throws Exception {
        ...
        try {
            // 调用 Feign 客户端获取会话信息
            ResponseEntity<String> response = bifroMQApi.getSessionInfo(Long.valueOf(IdUtil.getSnowflake().nextId()), tenantId, userId, clientId);
            log.info("Received response from MQTT broker for session info: {}", response.getBody());
            // 检查响应状态和内容
            if (response.getStatusCode().is2xxSuccessful()) {
                return JsonUtils.parseObject(Objects.requireNonNull(response.getBody()), MqttSessionDetailsResultVO.class);
            }
            log.warn("Failed to retrieve session info. Status: {}, tenantId: {}, userId: {}, clientId: {}",
                    response.getStatusCode(), tenantId, userId, clientId);
            // 非 2xx 但又不是 404 ── 走通用业务异常,由 isOnline 等调用方按 ServiceException 兜底处理为 R.fail
            throw new ServiceException("Failed to retrieve session info, status=" + response.getStatusCode());
            ...
            throw new ServiceException("Session not found."+e.getMessage());
     ...
     * {@link ServiceException} / 其它 {@link Exception} → 不确定(broker 临时不可达,调用方保留现状).
     */
    @Override
    ...
    @Override
    public void expireSession(String tenantId, String expirySeconds) throws ServiceException {
        log.info("Expiring sessions for tenantId: {} with expirySeconds: {}", tenantId, expirySeconds);
        try {
            ResponseEntity<String> response = bifroMQApi.expireSession(Long.valueOf(IdUtil.getSnowflake().nextId()), tenantId, expirySeconds);
            if (response.getStatusCode().is2xxSuccessful()) {
                ...
                throw new ServiceException("Failed to expire sessions for tenantId: " + tenantId);
            }
        } catch (Exception e) {
            log.error("Error occurred while expiring sessions: {}", e.getMessage());
            throw new ServiceException("Error during session expiration: " + e.getMessage());
        }
    }

    @Override
    public void killClientConnection(String tenantId, String userId, String clientId, String clientType) throws ServiceException {
        log.info("Killing client connection for tenantId: {}, userId: {}, clientId: {}, clientType: {}", tenantId, userId, clientId, clientType);
        try {
            ResponseEntity<String> response = bifroMQApi.killClientConnection(Long.valueOf(IdUtil.getSnowflake().nextId()), tenantId, userId, clientId, clientType, Collections.emptyMap());
            if (response.getStatusCode().is2xxSuccessful()) {
                ...
                throw new ServiceException("Failed to kill client connection for tenantId: " + tenantId);
            }
        } catch (Exception e) {
            log.error("Error occurred while killing client connection: {}", e.getMessage());
            throw new ServiceException("Error during client disconnection: " + e.getMessage());
        }
    }

    @Override
    public void addTopicSubscription(String tenantId, String userId, String clientId, String topicFilter, String subQos) throws ServiceException {
        ...
        try {
            ResponseEntity<String> response = bifroMQApi.addTopicSubscription(Long.valueOf(IdUtil.getSnowflake().nextId()), tenantId, userId, clientId, topicFilter, subQos);
            if (response.getStatusCode().is2xxSuccessful()) {
                ...
                throw new ServiceException("Failed to add topic subscription for tenantId: " + tenantId);
            }
        } catch (Exception e) {
            log.error("Error occurred while adding topic subscription: {}", e.getMessage());
            throw new ServiceException("Error during topic subscription: " + e.getMessage());
        }
    }

    @Override
    public void removeTopicSubscription(String tenantId, String userId, String clientId, String topicFilter) throws ServiceException {
        log.info("Removing topic subscription for tenantId: {}, userId: {}, clientId: {}, topicFilter: {}",
                tenantId, userId, clientId, topicFilter);
        try {
            ResponseEntity<String> response = bifroMQApi.removeTopicSubscription(Long.valueOf(IdUtil.getSnowflake().nextId()), tenantId, userId, clientId, topicFilter);
            if (response.getStatusCode().is2xxSuccessful()) {
                ...
                throw new ServiceException("Failed to remove topic subscription for tenantId: " + tenantId);
            }
        } catch (Exception e) {
            log.error("Error occurred while removing topic subscription: {}", e.getMessage());
            throw new ServiceException("Error during topic unsubscription: " + e.getMessage());
        }
    }

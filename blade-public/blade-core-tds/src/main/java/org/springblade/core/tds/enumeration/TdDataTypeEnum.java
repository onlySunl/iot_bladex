package org.springblade.core.tds.enumeration;
...
@Getter
@Slf4j
@Schema(title = "DataTypeEnum", description = "TD数据类型-枚举")
public enum TdDataTypeEnum {
    ...
    private static final ConcurrentHashMap<String, TdDataTypeEnum> DATA_TYPE_MAPPING = new ConcurrentHashMap<>();

    /**
     * 静态代码块，初始化映射关系
     * TODO 可以根据实际情况修改映射关系、新增类型需要在此处添加映射关系
     */
    static {
        // 标准映射关系
        for (TdDataTypeEnum type : values()) {
    ...
    TdDataTypeEnum(String dataType, boolean quoted) {
     ...
     */
    public static TdDataTypeEnum valueOfByDataType(String dataType) {
        if (dataType == null) {
     ...
     * @return 转换后的值
     */
    public static Object convertValue(String value, TdDataTypeEnum typeEnum) {
        if (value == null || typeEnum == null) {
            log.warn("Value or dataType is null. Treating as string.");
            return null;
        }
        try {
            switch (typeEnum) {
            ...
            // 转换失败时，记录警告并返回原始字符串值
            // 请确保log变量已经正确初始化
            log.warn("Data type conversion failed for value: '{}', dataType: '{}'. Treating as string.", value, typeEnum.getDataType(), e);
            return value;
        }
    }

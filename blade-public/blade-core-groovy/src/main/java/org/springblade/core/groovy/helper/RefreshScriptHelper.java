 ...
 * 刷新 groovy 脚本helper
 *
 * @author mqttsnet 2024/10/01 14:00
 */
@Slf4j
    ...
    public boolean refreshAll() {
        ...
        try {
            ...
            for (ScriptEntry scriptEntry : scriptEntries) {
                ScriptEntry oldScriptEntry = scriptRegistry.findOnCache(new ScriptQuery(scriptEntry.getUniqueKey()));

                // 缓存中没有，则注册进缓存
                if (Objects.isNull(oldScriptEntry)) {
                    log.info("can not found script by [{}], register to registry directly.", scriptEntry.getUniqueKey());
                ...
                if (!isEquals) {
                    log.info("found script by [{}], but their fingerprints are different, modify it.", scriptEntry.getUniqueKey());
                    // 编译脚本为Class

package org.springblade.core.groovy.registry.impl;

import com.github.benmanes.caffeine.cache.Cache;
import org.springblade.core.groovy.entity.ScriptEntry;
import org.springblade.core.groovy.entity.ScriptQuery;
import org.springblade.core.groovy.exception.LoadScriptException;
import org.springblade.core.groovy.exception.RegisterScriptException;
 ...
 * 默认注册中心
 *
 * @author mqttsnet 2025/03/18 11:49
 */
@Slf4j
public class DefaultScriptRegistry implements ScriptRegistry {
    ...
    public Boolean register(ScriptEntry scriptEntry) {
        ...
        try {
            // 强制覆盖
            cache.put(scriptEntry.getUniqueKey(), scriptEntry);
        } catch (Exception ex) {
    ...
    public Boolean register(ScriptEntry scriptEntry, boolean allowToCover) {
        // 旧entry
        ScriptEntry oldEntry = cache.getIfPresent(scriptEntry.getUniqueKey());
        ...
        log.error("can not register [{}], because [{}] already exists, please check.",
                scriptEntry.getUniqueKey(), scriptEntry.getLastModifiedTime());
        // 不覆盖，返回false
        return false;
    }
    ...
    public ScriptEntry find(ScriptQuery scriptQuery) throws Exception {
        ...
        synchronized (scriptQuery.getUniqueKey().intern()) {
            ...
            if (Objects.isNull(entry)) {
                ...
                // 设置脚本名称
                entry.setUniqueKey(scriptQuery.getUniqueKey());
                // 放入缓存
                if (!register(entry)) {
                    log.error("put ScriptEntry to cache failed, uniqueKey is [{}], lastModifiedTime is [{}], " +
                              "scriptContext is [{}]", entry.getUniqueKey(), entry.getLastModifiedTime(), entry.getScriptContext());
                }
            }
        }

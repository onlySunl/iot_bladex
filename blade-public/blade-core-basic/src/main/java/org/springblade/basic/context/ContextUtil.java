package org.springblade.basic.context;
...
public final class ContextUtil {
     ...
     * @create [2021/6/23 9:26 下午 ] [mqttsnet] [初始创建]
     */
...
//    private static final ThreadLocal<Map<String, String>> THREAD_LOCAL = new TransmittableThreadLocal<>();  // 配合 @Async 使用时会有问题
     ...
     * 存放每个租户的数据库名称
     */
    private static final Map<String, String> DATA_BASE = new ConcurrentHashMap<>();
    ...
    public static void setDataBase(String key, Object value) {
        ...
        DATA_BASE.put(key, value.toString());
    }
    ...
    public static String getDataBase(String key) {
        return DATA_BASE.getOrDefault(key, StrPool.EMPTY);
    }

    public static String getDataBase() {
        return DATA_BASE.getOrDefault(ContextConstants.TENANT_EXTEND_POOL_NAME_HEADER + StrPool.UNDERSCORE + ContextUtil.getTenantId(), StrPool.EMPTY);
    }
    ...
    /**
     * 租户 id(String 形态入参)
     * 无需调用方 {@code Long.valueOf(...)} 转换;空 / 空白字符串安全忽略,不污染上下文.
     *
     * @param tenantId 字符串形态租户 id
     */
    public static void setTenantIdStr(String tenantId) {
    ...
    public static void setDefTenantId() {
        ...
        set(ContextConstants.TENANT_EXTEND_POOL_NAME_HEADER, ContextConstants.DEF_TENANT_ID_SEQ);
    }
     ...
     * @create [2022/9/9 4:50 PM ] [mqttsnet] [初始创建]
     */

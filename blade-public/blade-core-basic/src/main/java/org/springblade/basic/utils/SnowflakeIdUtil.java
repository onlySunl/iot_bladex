...
public class SnowflakeIdUtil {
    ...
    /**
     * 生成一个新的唯一 ID（静态入口，数值型 {@code long}）。
     * <p>与 {@link #nextId()}（16 位十进制字符串）并列，但返回数值型 —— 适合协议里规定为数值的字段
     * （如 head.mid），避免被当成 / 序列化成字符串。
     *
     * @return 一个不重复的长整型值
     */
    public static long nextLong() {
        return INSTANCE.nextLongId();
    }
    ...
    public synchronized String nextId(int numDigits) {
        ...
        long id = nextLongId();
        return String.format("%0" + numDigits + "d", id % (long) Math.pow(10, numDigits));
    }
    ...
    public synchronized long nextLongId() {
        ...
        return ((timestamp - START_TIMESTAMP) << TIMESTAMP_SHIFT)
                  | ((long) datacenterId << DATACENTER_ID_SHIFT)

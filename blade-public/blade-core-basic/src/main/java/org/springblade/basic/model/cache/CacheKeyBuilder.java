package org.springblade.basic.model.cache;
 ...
 * 0）前缀： 可选。 用来区分不同项目，不同环境。 如：区分thinglinks-cloud 或 thinglinks-boot、thinglinks-cloud的dev或test环境
 * 1）租户编码： 可选。 用来区分不同租户数据缓存。 如： 内置租户和阿里巴巴租户分别用0000 和 1111 来区分。
